...
class Windows(Tk):
    ...
    def New_windows(self):
        ...
        btn_login = Button(self.win, image=self.win.login, activebackground="#0066CA", bg="#0066CA", bd=0,
                           command=self.LogWin)
        ...
        btn_register = Button(self.win, image=self.win.register, command=self.RegWin, activebackground="#0066CA",
                              bg="#0066CA", bd=0)
    ...
    def RegWin(self):
        ...
        def insert():
            ...
            try:
                contd = [txt_fname.get() != "", txt_lname.get() != "", txt_email.get() != "", txt_gender.get() != "",
                         txt_contactno.get() != "",
                         txt_password.get() != ""]
                ...
                if all(contd):
                    if x == 10:
                        ...
                        value = (
                            txt_fname.get(), txt_lname.get(), txt_email.get(), txt_gender.get(), txt_contactno.get(),
                            txt_password.get())
        ...
        text_field2 = Label(self.reg, image=self.reg.txt1, bg="#0066CA", bd=0).place(x=1005, y=320, width=270,
                                                                                     height=50)
        txt_lname = Entry(self.reg, font=("times new roman", 15), bg="#E9EDF5", bd=0)
        txt_lname.place(x=1017, y=325, width=245, height=40)
        # -------------------Row2

        text_field3 = Label(self.reg, image=self.reg.txt1, bg="#0066CA", bd=0).place(x=1005, y=450, width=270,
                                                                                     height=50)
        txt_gender = Entry(self.reg, font=("times new roman", 15), bg="#E9EDF5", bd=0)
        txt_gender.place(x=1017, y=455, width=245, height=40)

        text_field4 = Label(self.reg, image=self.reg.txt1, bg="#0066CA", bd=0).place(x=1005, y=600, width=270,
                                                                                     height=50)
        ...
        btn_register = Button(self.reg, image=self.reg.register, command=insert, activebackground="#0066CA",
                              bg="#0066CA", bd=0)
    ...
    def LogWin(self):
        ...
        # Adding Round Shape in Entry
        register_label = Label(self.log, image=self.log.txt1, bg="#0066CA", bd=0).place(x=650, y=520, width=320,
                                                                                        height=50)
    ...
    def Log_admin(self):
        ...
        def Login():
            ...
            try:
                myCur = myConn.mydb.cursor()
                query = "Select user_name,password from admin_user"
        ...
        # Adding Round Shape in Entry
        login_label = Label(self.log, image=self.log.txt1, bg="#0066CA", bd=0).place(x=650, y=370, width=320,
                                                                                     height=50)
        entry_one = Entry(self.log, border=0, font=("Times New Roman", 15))
        entry_one.config(bg="#E9EDF5")
        entry_one.place(x=662, y=375, width=290, height=40)
